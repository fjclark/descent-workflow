...
def get_all_mm_components(
    ...
    with ctx.Pool(processes=n_workers) as pool:
        ...
        for filtered_components, n_total, n_filt in tqdm(
            results_iter, total=lx, desc="Extracting MM components", unit="mol"
        ):
...
def get_all_mm_components_by_type_parallel(
    ...
    with ctx.Pool(processes=n_workers) as pool:
        ...
        results_iter = pool.imap_unordered(get_smirks_fn, mm_components, chunksize=100)

        # Wrap with tqdm for progress tracking
        for smirks, component in tqdm(
            results_iter, total=total, desc="Grouping by SMIRKS", unit="component"
        ):
            all_component_types[smirks].append(component)
        ...
        if i < len(specificity_order) - 1:
            ...
            too_specific = [
                ...
                for component_list in tqdm(
                    components_by_type.values(),
                    total=len(components_by_type),
                    desc="Filtering rare types",
                    unit="type",
                )
            ...
            # Remove SMIRKS patterns below cutoff from current level
            smirks_to_remove = [
                s for s, cs in components_by_type.items() if len(cs) < cutoff_population
            ]
            for smirks in smirks_to_remove:
                del components_by_specificity[specificity_num][smirks]

            # Delete smirks_to_remove list to free memory
            del smirks_to_remove, components_by_type
            gc.collect()
